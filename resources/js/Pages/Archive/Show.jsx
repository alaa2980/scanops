...
export default function ShowArchiveDetails({ scan, detections, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('archive.show', scan.id), { dq: searchQuery }, { preserveState: true });
    };

    const handleRefresh = () => {
        router.reload({ only: ['scan', 'detections'] });
    };
    ...
    return (
        <AppLayout>
            <Head title={`Report #${scan.reference_code || scan.id}`} />
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                        <div>
                            ...
                            <h1 className="text-3xl font-extrabold tracking-tight text-white mb-2">
                                Reference #{scan.reference_code || scan.id}
                            </h1>
                            <p className="text-sm text-slate-400 mb-4 max-w-xl">
                                Review the comprehensive analysis summary, processing timeline, and detailed target detection logs for this sector.
                            </p>
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-300">
                                ...
                                {scan.started_at && (
                                    <span className="rounded-xl border border-white/10 bg-black/40 px-3 py-2">
                                        Started: {new Date(scan.started_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}
                                    </span>
                                )}
                                {scan.finished_at && (
                                    <span className="rounded-xl border border-white/10 bg-black/40 px-3 py-2">
                                        Finished: {new Date(scan.finished_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}
                                    </span>
                                )}
                            </div>
                        </div>
                ...
                <div className="bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-3xl shadow-xl overflow-hidden">
                    <div className="p-6 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-800/20">
                        <div>
                            ...
                            <p className="text-xs text-slate-400 mt-1">Detailed coordinates and confidence scores.</p>
                        </div>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-12 text-center text-slate-500">
                                            <div className="inline-block p-4 rounded-2xl bg-black/20 border border-dashed border-white/10">
                                                No detections found for this sector.
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                    ...
                    {detections.links && detections.data.length > 0 && (
                        <div className="p-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-800/20">
                            ...
                            <div className="flex items-center gap-2">
                                <Link
                                    href={detections.prev_page_url || '#'}
                                    className={`px-4 py-2 rounded-xl text-sm font-semibold border border-white/10 transition-colors ${detections.prev_page_url ? 'bg-white/5 text-slate-200 hover:bg-white/10' : 'bg-transparent text-slate-600 cursor-not-allowed'}`}
                                >
                                    Previous
                                </Link>
                                <Link
                                    href={detections.next_page_url || '#'}
                                    className={`px-4 py-2 rounded-xl text-sm font-semibold border border-white/10 transition-colors ${detections.next_page_url ? 'bg-white/5 text-slate-200 hover:bg-white/10' : 'bg-transparent text-slate-600 cursor-not-allowed'}`}
                                >
                                    Next
                                </Link>
                            </div>
